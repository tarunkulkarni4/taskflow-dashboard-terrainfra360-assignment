...
const LightPillar = ({
  ...
  mixBlendMode = 'screen',
  pillarRotation = 0,
  quality = 'medium' // Optimized default to 'medium' to avoid GPU throttle
}) => {
  ...
  useEffect(() => {
    ...
    const qualitySettings = {
      low: { iterations: 20, waveIterations: 1, pixelRatio: 0.4, precision: 'mediump', stepMultiplier: 1.6 },
      medium: { iterations: 36, waveIterations: 2, pixelRatio: 0.6, precision: 'mediump', stepMultiplier: 1.3 },
      high: {
        iterations: 70,
        waveIterations: 3,
    ...
    const startAnimate = () => {
      if (!isLooping && isVisibleRef.current) {
        isLooping = true;
        rafRef.current = requestAnimationFrame(animate);
      }
    };

    // IntersectionObserver to pause loop if panel is display: none (e.g. mobile)
